...
class PatternPlaceholder(Pattern):
    """Pattern for Python object placeholders used by the template lexer."""
    ...
    def to_regexp(self) -> str:
        # Use an always-failing regexp to ensure the basic lexer never matches
        # these placeholders from textual input.
        return r'(?!.*)'
...
class PatternTree(Pattern):
    """Pattern for template tree splicing terminals."""
...
def _create_unless(terminals, g_regex_flags, re_, use_bytes):
    relevant = [t for t in terminals if isinstance(t.pattern, (PatternRE, PatternStr))]
