...
class PatternPlaceholder(Pattern):
    """Pattern that represents an interpolated Python object.

    These patterns never match actual text. They exist so the Earley
    template frontend can recognise placeholder terminals supplied by
    template literals.
    """
    ...
    def to_regexp(self) -> str:
        # An impossible pattern that will never match actual text
        return r'(?!.)'
...
class PatternTree(Pattern):
    """Pattern that stands for a pre-built Tree spliced into the input."""
