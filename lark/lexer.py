...
class PatternPlaceholder(Pattern):
    """Pattern for template PYOBJ placeholders."""
    ...
    def to_regexp(self) -> str:
        # Intentionally impossible pattern so the regex engine never matches it.
        return r'(?!.*)'
...
class PatternTree(Pattern):
    """Pattern for template tree interpolation tokens."""
    ...
    def __init__(self, conf: 'LexerConf', comparator=None) -> None:
        ...
        if not conf.skip_validation:
            ...
            for t in terminals:
                ...
                if t.pattern.type not in {"placeholder", "tree"} and t.pattern.min_width == 0:
