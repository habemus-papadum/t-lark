Below is a final, implementation‑oriented plan for adding a template mode to Lark that parses Python 3.13 string.templatelib.Template objects (your “t‑strings”) with:

Interpolated Python strings treated as standalone nodes/tokens (not merged into surrounding text).

Always-available subtree splicing: you can splice in a pre‑existing Lark Tree anywhere it fits the grammar, even if the grammar contains no ∏ placeholders.

∏ (Pi) only denotes Python objects (non‑Tree) in the grammar.

The plan is written for a future you (or your tech lead) to implement directly.
...
Internally, the grammar is auto‑augmented with tiny injection rules that let any nonterminal accept a prebuilt subtree whose label matches what the grammar would normally produce there. (Details in the steps.)
...
static segment metadata (filename, offsets), and
...
Keep core algorithms unmodified; hook via frontends and small grammar augmentation.
...
After the loop, lex and yield tokens for the final trailing static string (if any).

Important revision honored: Interpolated Python strings are not merged into adjacent static text; they appear as PYOBJ tokens. If someone wants merging semantics, they should use plain t‑strings outside this mode.
...
Unexpected object where grammar has no ∏ → Earley raises with token position; we can wrap to say:
“Interpolated object at {file}:{line}:{col} not allowed here (no ∏ placeholder in this context).”

Tree label not accepted: if a Tree('foo') appears where the expected nonterminal N does not list foo in labels(N), the derivation won’t match; Earley error will point to that token’s source. The message can include “tree label ‘foo’ not valid for nonterminal ‘N’” if you enhance the error reporter (optional).
