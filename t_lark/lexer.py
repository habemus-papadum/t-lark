...
class PatternPlaceholder(Pattern):
    """Pattern object for template placeholders."""
    ...
    @property
    def min_width(self) -> int:
        # Report non-zero width to bypass basic lexer validation; pattern never matches text anyway.
        return 1
...
class PatternTree(Pattern):
    """Pattern object for template tree splicing."""
    ...
    @property
    def min_width(self) -> int:
        # Non-zero width avoids validation errors; these terminals are never matched via regex.
        return 1
    ...
    def __init__(self, conf: 'LexerConf', comparator=None) -> None:
        ...
        if not conf.skip_validation:
            ...
            if has_interegular:
                ...
                raise LexError("interegular must be installed for strict mode. Use `pip install 't_lark[interegular]'`.")
