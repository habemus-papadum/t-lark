...
class PatternPlaceholder(Pattern):
    """Pattern used for template PYOBJ placeholders."""
    ...
    def to_regexp(self) -> str:
        # Never matches via regex – handled manually in template mode
        return r'(?!.*)'
...
class PatternTree(Pattern):
    """Pattern used for template tree splicing placeholders."""
...
def _create_unless(terminals, g_regex_flags, re_, use_bytes):
    ...
    embedded_strs = set()
    callback = {}
    regex_tokens = tokens_by_type.get(PatternRE, [])
    str_tokens = tokens_by_type.get(PatternStr, [])
    for retok in regex_tokens:
        unless = []
        for strtok in str_tokens:
